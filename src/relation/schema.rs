//! Relation schema's
//!
//! The [NamedStruct] type isn't easy to use, so this module defines a [Schema]
//! type that is a bit easier to handle that converts from a NamedStruct.
//!

use proto::r#type::Nullability;

/// A collection of ordered named fields.
pub struct Schema<'a> {
    fields: Vec<Field<'a>>,
    // type_variation: &'a SimpleExtensionsTypeVariationsItem,
...
impl NamedStruct {
    fn as_schema(&self) -> Schema {
        ...
        Schema {
            fields: self.r#struct.fields.iter().map(|r#type| Field {
                name: names.next().unwrap(),
                r#type,
            }),
            nullability: Nullability::from_i32(self.nullability).unwrap(),
        }
    }
}
